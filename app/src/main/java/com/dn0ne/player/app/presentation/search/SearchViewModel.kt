...
class SearchViewModel(
    ...
    fun resolveStreamUrl(track: Track, onResolved: (String) -> Unit) {
        ...
        if (track.data.startsWith("http")) {
            ...
            android.util.Log.d("SearchViewModel", "Resolving stream for track: ${track.title}, data: ${track.data}, videoId: $videoId")
            if (videoId == track.data) {
                android.util.Log.d("SearchViewModel", "VideoId same as data, using as is")
            ...
            viewModelScope.launch {
                ...
                result.onSuccess { url ->
                    android.util.Log.d("SearchViewModel", "Stream URL resolved successfully: $url")
                   onResolved(url)
                }.onFailure {
                    android.util.Log.e("SearchViewModel", "Stream URL resolution failed, using original: ${it.message}")
            ...
            android.util.Log.d("SearchViewModel", "Local track, using data directly: ${track.data}")
            onResolved(track.data)
        }
    }
}
