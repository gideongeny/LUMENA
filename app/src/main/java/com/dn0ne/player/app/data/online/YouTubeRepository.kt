...
class YouTubeRepository(private val context: android.content.Context) {
    ...
    init {
        try {
            ...
            // YoutubeDL native assets are not bundled, skip initialization
            android.util.Log.d("YouTubeRepository", "YoutubeDL not available in this build, will rely on InnerTube/NewPipe")
                ...
                if (item is org.schabi.newpipe.extractor.stream.StreamInfoItem) {
                    try {
                        ...
                        val mediaMetadata = MediaMetadata.Builder()
                            .setTitle(item.name)
                            .setArtist(item.uploaderName ?: "Unknown Artist")
                            .setAlbumTitle("YouTube")
                            .setArtworkUri(android.net.Uri.parse(normalizedThumbnail))
                            .build()
    ...
    private suspend fun searchWithInnerTubeAPI(query: String): List<Track> {
        return try {
            ...
            // Log truncated response for debugging and save full response to cache for offline inspection
            try {
                val truncated = if (response.length > 4000) response.substring(0, 4000) + "..." else response
                android.util.Log.d("YouTubeRepository", "InnerTube raw response (search) for query: $query: $truncated")
                val timestamp = System.currentTimeMillis()
                val fileName = "innertube_search_${'$'}{timestamp}.json"
                try {
                    val f = java.io.File(context.cacheDir, fileName)
                    f.writeText(response)
                    android.util.Log.d("YouTubeRepository", "Wrote InnerTube response to: ${f.absolutePath}")
                } catch (ioe: Exception) {
                    android.util.Log.w("YouTubeRepository", "Failed to write InnerTube response to cache", ioe)
                }
            } catch (e: Exception) {
                android.util.Log.w("YouTubeRepository", "Failed to log InnerTube response", e)
            }
            ...
            try {
                ...
                if (url != null) {
                    if (isPlayableUrl(url)) {
                        ...
                        android.util.Log.w("YouTubeRepository", "ANDROID_MUSIC returned non-playable URL, continuing")
                    }
                }
            } catch (e: Exception) {
            ...
            try {
                ...
                if (url != null) {
                    if (isPlayableUrl(url)) {
                        ...
                        android.util.Log.w("YouTubeRepository", "TVHTML5SIMPLY_EMBEDDED returned non-playable URL, continuing")
                    }
                }
            } catch (e: Exception) {
            ...
            try {
                ...
                if (url != null) {
                    if (isPlayableUrl(url)) {
                        ...
                        android.util.Log.w("YouTubeRepository", "IOS returned non-playable URL, continuing")
                    }
                }
            } catch (e: Exception) {
            ...
            try {
                ...
                if (url != null) {
                    if (isPlayableUrl(url)) {
                        ...
                        android.util.Log.w("YouTubeRepository", "ANDROID_TESTSUITE returned non-playable URL, continuing")
                    }
                }
            } catch (e: Exception) {
            ...
            try {
                ...
                if (url != null) {
                    if (isPlayableUrl(url)) {
                        ...
                        android.util.Log.w("YouTubeRepository", "ANDROID returned non-playable URL, continuing")
                    }
                }
            } catch (e: Exception) {
            ...
            try {
                ...
                if (url != null) {
                    if (isPlayableUrl(url)) {
                        ...
                        android.util.Log.w("YouTubeRepository", "WEB returned non-playable URL, continuing")
                    }
                }
            } catch (e: Exception) {
            ...
            try {
                ...
                if (bestStream != null) {
                    ...
                    if (isPlayableUrl(url)) {
                        ...
                        android.util.Log.w("YouTubeRepository", "NewPipe returned non-playable URL, continuing")
                    }
                }
            } catch (e: Exception) {
    ...
    private suspend fun getStreamUrlWithYoutubeDL(videoId: String): String? {
        // YoutubeDL not available; use InnerTube or NewPipe instead
        return null
    }
    ...
    private fun getStreamUrlWithInnerTube(videoId: String, clientType: String): String? {
        try {
            ...
            if (playabilityStatus != null) {
                ...
                android.util.Log.d("YouTubeRepository", "InnerTube response ($clientType): status=$status, reason=$reason")
                if (status != "OK") {
            ...
            if (streamingData == null) {
                android.util.Log.e("YouTubeRepository", "No streamingData in response ($clientType)")
                return null
            }
            android.util.Log.d("YouTubeRepository", "Found streamingData ($clientType)")
              ...
              if (hlsManifestUrl.isNotEmpty()) {
                  ...
                  if (isPlayableUrl(hlsManifestUrl)) return hlsManifestUrl
                  android.util.Log.w("YouTubeRepository", "HLS manifest not playable, continuing: $hlsManifestUrl")
              }
              ...
              if (dashManifestUrl.isNotEmpty()) {
                  ...
                  if (isPlayableUrl(dashManifestUrl)) return dashManifestUrl
                  android.util.Log.w("YouTubeRepository", "DASH manifest not playable, continuing: $dashManifestUrl")
              }
            ...
            if (formats != null) {
                for (i in 0 until formats.length()) {
                    ...
                    if (!url.isNullOrEmpty()) {
                        ...
                        android.util.Log.w("YouTubeRepository", "Format URL not playable, skipping: $url")
                    }
                    // Handle cipher
            ...
            if (adaptiveFormats != null) {
                ...
                for (format in audioFormats) {
                    ...
                    if (url.isNotEmpty()) {
                        if (isPlayableUrl(url)) return url
                        android.util.Log.w("YouTubeRepository", "Adaptive audio format not playable, skipping: $url")
                    }
    ...
    private fun isPlayableUrl(url: String): Boolean {
        try {
            // Quick HEAD check to avoid passing HTML/error pages to ExoPlayer
            val conn = URL(url).openConnection() as java.net.HttpURLConnection
            conn.requestMethod = "HEAD"
            conn.setRequestProperty("User-Agent", LumenaDownloader.USER_AGENT)
            ...
            if (code >= 400) {
                android.util.Log.w("YouTubeRepository", "HEAD check failed code=$code for $url")
                return false
            }
            if (contentType.contains("text/html")) {
                android.util.Log.w("YouTubeRepository", "HEAD check HTML content for $url")
                return false
            }
            if (contentType.contains("mpegurl") || contentType.contains("application/vnd.apple.mpegurl") || contentType.contains("application/dash+xml") || contentType.startsWith("audio") || contentType.contains("octet-stream")) {
                return true
            }
            if (url.contains("watch?v=") || url.contains("/watch?")) return false
            // fallback: accept if not HTML and not clearly a watch page
            return true
        } catch (e: Exception) {
            android.util.Log.w("YouTubeRepository", "isPlayableUrl check failed for $url", e)
            return false
        }
    }
}
